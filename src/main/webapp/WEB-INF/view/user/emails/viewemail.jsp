	...
	<div class="row">
		<div class="col-sm-8 col-md-9">
			<ul class="menus">
				...
				<li class="menu-item">
				  <a href="${contextPath}/user/reply-email?emailId=${email.emailId}"><i class="fa fa-file-picture-o icon-menu"></i>Reply</a>
				</li>
				<li class="menu-item">
...
function deleterecieveremail(emailId){
	BootstrapDialog.confirm('Are you sure! You want Delete this Email Information?', function(result){
        if(result) {
        	$.get("${contextPath}/user/receiver-email-delete-by-id?emailId="+emailId).done(function(data){
        		if(data.status=="success"){
        			 BootstrapDialog.alert(data.message, function(alertresult){
        				 if(alertresult){
        				  $(location).attr('href',"${contextPath}/user/emailinbox");
        				 }
        			 });
        		}
        	});
